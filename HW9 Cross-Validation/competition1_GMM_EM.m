...
% mean log-likelihood of Kernel method is GMM_EM.
...
%% Compare Histogram model with Gaussian Model
clear
...
n = 16; % n fold corss-validation
d = 6; % number of Gaussian components.
width= length(X_train)/n;
edge2 = edge2(1,1:end-1)+diff(edge2,1)/2;
[psor,lsor] = findpeaks(c2,edge2,'MinPeakDistance',0.6,'SortStr','descend'); % use peaks as initial mean guess.
...
%   initial guess for each fold
    
mu(fold,:) = lsor(1:d); % the mean in descending order.

var(fold,:) = 0.1*rand(1,length(mu(1,:))); 

wei(fold,:) = psor(1:d)./sum(psor(1:d));


for iter = 1:6 %% EM iteration cycles
...
% Expectation step
    ...
    for j = 1:d
     F(j,:) = wei(fold,j).*(1/(sqrt(2*pi*var(j)))).*exp(-0.5*(Subtrain(fold,:)-mu(fold,j)).^2./var(fold,j));
    end
...
for j = 1:d
wei(fold,j) = G(j)/(length(X_train)*((n-1)/n));

mu(fold,j) = sum(R(j,:).* Subtrain(i,:),2)./G(j);

var(fold,j) = sum( R(j,:).*Subtrain(fold,:).*Subtrain(fold,:)./G(j),2);
end
end
end


%% getting estimated datapoint probability
clear Z
for fold = 1: n
...
for fold = 1:n
gmm(fold)= sum(log(Z(fold,:)))/length(Subset(fold,:));
...
Y_tt= sum(Y,1);
CV(fold) = sum( ((c4-Y_tt).^2).*Y_tt); %% use subset histogram as benchmark.
end
...
hAx(1).YGrid = 'on';
legend('True','GMM');
str1 = sprintf('testset %d hist & &d th subtrain GMM',fold,fold);
title({str1});
...
str1 = sprintf('X_train hist. & %d th fold GMM & Gaussian ',fold);
title({str1});
