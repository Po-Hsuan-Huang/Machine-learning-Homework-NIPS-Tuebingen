...
C_rotate(:,:,fold) = V'*C_super(:,:,fold)*V;
...
for fold = 1:n
    ...
    test = T(fold);
   sum1= mean(( C_tr(:,:,test)-C_rotate(:,:,fold)).^2,1);
...
for i = 1:width
   ...
   ll_rotate(i)= -0.5*(SubX(:,i,test)-mu)'*B*(SubX(:,i,test)-mu);
end
LL_rotate(fold)=( -100*log(sqrt(2*pi)) -0.5*log(det(A))  + sum(ll_rotate))/width;
LL_ml(fold)   =( -100*log(sqrt(2*pi)) -0.5*log(det(B))+ sum(ll_ml))/width;

end

LL_rotate_tt= mean(LL_rotate);
LL_ml_tt= mean(LL_ml);

alles = LL_rotate_tt-LL_ml_tt;
