...
%% Task 3(b) With constant non-Gaussian pdf.
...
lamda = sqrt (S/2); % lamda is the parameter for laplace distribution lambda/2 * exp(-lambda*abs(x))
...
predict(1,m+i) =  pre + sqrt(S)*randlap(1,lamda);  % prediction with Gaussain Noice 
